...
class OpenTelemetryFactory
{
    ...
    protected function createNoOpTracer(): TracerInterface
    {
        return new class implements TracerInterface {
			...
			// Find the spanBuilder method in the anonymous class
			public function spanBuilder(string $name): \OpenTelemetry\API\Trace\SpanBuilderInterface
			{
				// If the original tracer has a spanBuilder method, delegate to it
				if (method_exists($this->tracer, 'spanBuilder')) {
					// Make sure we're returning the correct interface type
					return $this->tracer->spanBuilder($name);
				}
				
				// If you need a fallback implementation, create one that implements SpanBuilderInterface
				// For example:
				return new \OpenTelemetry\SDK\Trace\SpanBuilder($name, $this->tracer);
				
				// If you're not sure how to create a valid SpanBuilder, you might need to
				// check if there's a factory method in the OpenTelemetry SDK
			}
			...
			public function isEnabled(): bool {
				// If your wrapped tracer also implements isEnabled(), delegate to it
				if (method_exists($this->tracer, 'isEnabled')) {
					return $this->tracer->isEnabled();
				}
				
				// Default implementation - typically tracers are enabled by default
				return true;
			}
        };
    }
